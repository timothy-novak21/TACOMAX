...
if RUN_MIRROR_SWEEP:
    ...
    sim_rm_sweep_mirror(Rm,Bmax,v0,N,CONST)
...
if RUN_MIRROR_SINGLE:
    ...
    mode = "single"
    cycles = 2
    state = sim_single_mirror(mirror,theta,v0,mode,cycles,CONST)
    plot_single_mirror(state,mirror,theta) # plot single
...
if RUN_TOROIDAL_SWEEP:
    ...
    sim_pitch_sweep_tokamak(toroid,v0,N,CONST)
...
if RUN_TOROIDAL_SINGLE:
    ...
    mode = "single"
    cycles = 1
    state = sim_single_tokamak(toroid,theta,v0,mode,CONST,cycles)
...
if RUN_POLOIDAL_SINGLE:
    ...
    mode = "single"
    cycles = 1
    state = sim_single_tokamak(poloid,theta,v0,mode,CONST,cycles)
...
if RUN_Q_SAFETY_SWEEP:
    ...
    theta = np.radians(20) # pitch angle [rad]
    sim_q_sweep_tokamak(q_list,R0,r0,B0,v0,theta,CONST)
...
tokamak = Tokamak(R0,r0,B0,2.0) # initialize tokamak object

if RUN_PASSING_ORBIT:
    theta = np.radians(20) # pitch angle [rad]
    mode = "orbit"
    state = sim_single_tokamak(tokamak,theta,v0,mode,CONST)
    width = orbit_width(state)
    print(f"Orbit width: {width:.4f} m")
    plot_RZ(state,tokamak,theta)

if RUN_BANANA_ORBIT:
    theta = np.radians(80) # pitch angle [rad]
    mode = "orbit"
    state = sim_single_tokamak(tokamak,theta,v0,mode,CONST)
    width = orbit_width(state)
    print(f"Orbit width: {width:.4f} m")
